...
class InvertedResidual(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio):
        ...
        self.conv = nn.Sequential(
            ...
            ODConv2d(inp, inp * expand_ratio,
                      kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(inp * expand_ratio),
            nn.ReLU6(inplace=True),
            # depthwise convolution via groups
            ODConv2d(inp * expand_ratio, inp * expand_ratio,
                      kernel_size=3, stride=stride, padding=1, groups=inp * expand_ratio),
            nn.BatchNorm2d(inp * expand_ratio),
            nn.ReLU6(inplace=True),
            # pointwise linear convolution
            ODConv2d(inp * expand_ratio, oup,
                      kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(oup),
        )
...
#     print(net)
#     output, maps = net(torch.randn(2, 3, 224, 224))
